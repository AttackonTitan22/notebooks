...
帧的大小是等于页的大小的，每一个程序（进程)被划分成多个块，这每个块就是一个页面，形成了一个页表，多个程序多个页表。然后反向页表就是拿物理地址实际大小的所有帧来作为索引，这样所有的程序都可以共用一个反向页表。然后的话，从逻辑地址到物理地址，还是从页号找帧号，帧号+偏移=物理地址来计算。基于哈希(hash)的方案就是先根据页号通过哈希函数得到一个帧号，然后在反向列表中进行比对，哈希函数增加了一个PID作为辨识，如果帧号对应页号以及PID都相同，那这就是被找的帧号。
...
把程序按照其自身逻辑结构, 划分为若干个功能上**相互独立的程序模块**, 那些不会同时执行的模块共享同一块内存区域, 按时间先后来运行.
...
如果是程序太多, 超过了内存的容量, 可以采用自动的交换(swapping)技术, 把暂时**不能执行的程序**送到外存中

目的 : 多道程序在内存时, 让正在运行的程序或需要运行的程序获得更多的内存资源

原理 :

可将暂时不能运行的**程序**送到外存, 从而获得空闲内存空间. 操作系统把一个进程的整个地址空间的内容保存到外存中(换出 swap out), 而将外存中的某个进程的地址空间读入到内存中(换入 swap in). 换入换出内容的大小为整个程序的地址空间.
...
-   覆盖只能发生在那些相互之间没有调用关系的**程序模块之间**, 因此程序员必须给出程序内的各个模块之间的逻辑覆盖结构.
-   交换技术是以在内存中的程序大小为单位进行的, 它不需要程序员给出各个模块之间的逻辑覆盖结构.
-   换言之, 交换发生在内存中**程序与管理程序或操作系统之间**, 而覆盖则发生在**运行程序的内部**.
...
-   目标
    ...
    像覆盖技术那样, 不是把程序的所有内容都放在内存中, 因而能够运行比当前的空闲内存空间还要大的程序. 但做的更好, 由**操作系统自动来完成**, 无需程序员的干涉.
...
-   基本概念
    ...
    -   在装入程序时, 不必将其全部装入内存, 而只需将**当前需要执行的部分**页面或段装入到内存中, 就可以让程序开始执行;
...
-   基本特征
    ...
    -   大的用户空间 : 通过把**物理内存和外存**相结合, 提供给用户的虚拟内存空间通常大于实际的物理内存, 即实现了这两者的分离. 如32位的虚拟地址理论上可以访问4GB, 而可能计算机上仅有256M的物理内存, 但硬盘容量大于4GB.
    -   部分交换 : 与交换技术相比较, 虚拟存储的调入和调出是对**部分虚拟地址空间**进行的;
...
-   虚拟页式内存管理
    ...
    -   大部分虚拟存储系统都采用虚拟页式存储管理技术, 即在页式存储管理的基础上, 增加**请求调页**和**页面置换功能**.
    ...
    -   页表表项
        ...
        修改位 : 表示此页在内存中是否被修改过. 当系统回收该物理页面时, 根据此位来决定是否把它的内容**写回外存**
    ...
    -   缺页中断处理过程 :
        ...
        5.  修改p所对应的页表项的内容, 把驻留位置为1, 把**物理页帧号置为f**;
        6.  重新运行被中断是指令.
    ...
    -   虚拟内存性能
        ...
        EAT = 10\*(1-p) + 5000000\*p\*(1+q)     (1+q)表示在原本磁盘访问的基础之上还要加上修改了内存段写入到磁盘的概率
...
目标 : 尽可能地减少页面的换进换出次数(即缺页中断的次数). 具体来说, 把未来不再使用的或短期内较少使用的页面换出, 通常只能在局部性原理指导下**依据过去**的统计数据来进行预测.
...
这是一种**理想情况**, 在实际系统中是无法实现的, 因为操作系统无法知道每一个页面要等待多长时间以后才会再次被访问.

可用作其他算法的**性能评价的依据**.(在一个模拟器上运行某个程序, 并记录每一次的页面访问情况, 在第二遍运行时即可使用最优算法)
...
-   ![image-20230317160335903](C:\Users\DELL\AppData\Roaming\Typora\typora-user-images\image-20230317160335903.png)
...
![image-20230317162741315](C:\Users\DELL\AppData\Roaming\Typora\typora-user-images\image-20230317162741315.png)

### 二次机会算法

因为考虑到时钟页面置换算法, 有时候会把一些 dirty bit 为1(有过写操作)的页面进行置换, 这样的话, 代价会比较大. 因此, 可以结合**访问位**和**脏位**一起来决定应该置换哪一页.
...
![image-20230317165551473](C:\Users\DELL\AppData\Roaming\Typora\typora-user-images\image-20230317165551473.png)
...
在采用FIFO算法时, 有时会出现分配的**物理页面数增加, 缺页率反而提高的异常现象**;
...
-   如果局部性原理是成立的, 那么如何**来证明它的存在**, 如何来对它进行定量地分析? 这就是工作集模型.
...
![image-20230317210513442](C:\Users\DELL\AppData\Roaming\Typora\typora-user-images\image-20230317210513442.png)
...
-   优缺点 : 性能较好, 但增加了系统开销.
-   具体实现 : 可以使用缺页率算法来**动态调整常驻集的大小**.

缺页率 : 表示 "缺页次数 / 内存访问次数"

影响因素 : 页面置换算法, 分配给进程的物理页面数目, 页面本身的大小, 程序的编写方法.

![image-20230317211731076](C:\Users\DELL\AppData\Roaming\Typora\typora-user-images\image-20230317211731076.png)
...
-   产生抖动的原因 : 随着驻留内存的进程数目增加, 分配给每个进程的物理页面数不断就减小, 缺页率不断上升. 所以OS要选择一个适当的**进程数目**和进程需要的**帧数**, 以便在并发水平和缺页率之间达到一个平衡.

![image-20230317214336929](C:\Users\DELL\AppData\Roaming\Typora\typora-user-images\image-20230317214336929.png)
...
**并发性** : 进程可以被独立调度并占用处理机运行; (并发:一段时间, 并行:一时刻时间)

**独立性** : 不同进程的工作不相互影响;(页表是保障措施之一，虚拟内存映射不同的物理内存)
...
操作系统为每个进程都维护了一个PCB, 用来保存与该进程有关的**各种状态信息**.
...
![image-20230319171618504](C:\Users\DELL\AppData\Roaming\Typora\typora-user-images\image-20230319171618504.png)
...
进程在挂起状态时, 意味着进程**没有占用内存空间**, 处在挂起状态的进程**映像在磁盘上**.(把进程放到磁盘上)
...
-   ![image-20230319172117337](C:\Users\DELL\AppData\Roaming\Typora\typora-user-images\image-20230319172117337.png)
...
-   线程所需的资源
    ...
    ![image-20230319173627289](C:\Users\DELL\AppData\Roaming\Typora\typora-user-images\image-20230319173627289.png)
...
操作系统只能看到进程, 看不到线程, 线程的TCB在**线程库中实现**;

在用户空间实现的线程机制, 它不依赖于操作系统的内核, 由一组用户级的线程库来完成线程的管理, 包括线程的创建,终止,同步和调度等.

-   由于用户线程的维护由相应的进程来完成(通过**线程库函数**),不需要操作系统内核了解用户进程的存在,可用于不支持线程技术的多进程操作系统;
...
它允许一个进程指定**参数的数量**(argc)和它字符串**参数数组**(argv)
...
-   这个系统调用:
    ...
    -   释放大部分支持进程的操作系统结构
    -   检查是否父进程是存活着的:
        -   如果是的话,它保留结果的值直到父进程需要它;在这种情况里,进程没有真正死亡,但是它进入了僵尸状态(在回收过程中并未死亡，同时不属于其他任何状态)
        -   如果没有,它释放所有的数据结构,这个进程死亡
    -   清理所有等待的僵尸进程
-   进程终止是最终的垃圾收集(资源回收，根进程定期清理存在的僵尸进程)
-   ![image-20230319215405282](C:\Users\DELL\AppData\Roaming\Typora\typora-user-images\image-20230319215405282.png)
...
- **SPN(SJF) SRT(短进程优先(短作业优先)短剩余时间优先)[最优平均等待时间]**
  ...
  ![image-20230320170006841](C:\Users\DELL\AppData\Roaming\Typora\typora-user-images\image-20230320170006841.png)
...
2.  用二进制信号量实现的**调度约束**
    ...
    //Thread A
    ...
    condition->P(); //等待线程B某一些指令完成之后再继续运行,在此阻塞，此时信号量为-1
    ...
    
    //Thread B
    ...
    condition->V(); //信号量增加唤醒线程A，信号量加1后变成0，唤醒Thread A
    ...
    ```
...
3.  一个线程等待另一个线程处理事情
    ...
    BoundedBuffer::Deposit(c){
    		emptyBuffers->P();  //emptyBuffers->P()与mutex->P()互换位置，会导致死锁
    		mutex->P();         //因为如果生产者将buffer填满以后，mutex->P()能够进入buffer，而emptyBuffers->P()因为变成-1而睡眠，导致生产者在缓冲区中睡眠，当有消费者去buffer进行取操作的时候，buffer被生产者睡眠占用临界区而导致不能进行取操作，造成死锁现象。
    		Add c to the buffer;
    		mutex->V();
    		fullBuffers->V();
    }
...
**管程解决生产者-消费者问题的理解**：首先生产者如果将buffer填满，即count==n的时候，那么进入notFull.Wait(&lock)中，先将这个要休眠的进程的lock先release掉，这样再schedule（），schedule的含义是切换进程函数，也就是如果是新的生产者那就依然休眠，如果是消费者的Remove那么，就count--，这样的话就可以通知当时在休眠的生产者被唤醒用wakeup（t），然后signal（）跳到notFull.Wait(&lock)中的schedule（）之后继续执行。

![image-20230401232105937](C:\Users\DELL\AppData\Roaming\Typora\typora-user-images\image-20230401232105937.png)
...
- 当一个进程请求可用资源,系统必须判断立即分配是否能使系统**处于安全状态**
...
- 
- ![image-20230509162714656](C:\Users\DELL\AppData\Roaming\Typora\typora-user-images\image-20230509162714656.png)![](C:\Users\DELL\AppData\Roaming\Typora\typora-user-images\image-20230509163836057.png)![image-20230509163918739](C:\Users\DELL\AppData\Roaming\Typora\typora-user-images\image-20230509163918739.png)
